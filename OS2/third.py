...
class State: # 負責整體的狀態
    ...
    def schedual(self, x, readyQueue):
        if x and self.cpuProcess.cpuBurst > 0:
    ...
    def hasGetNext(self):
        if len(self.readyQueue) != 0:
            if self.cpuProcess.ID == -1:
                ...
                return True
            elif self.readyQueue[0].cpuBurst < self.cpuProcess.cpuBurst:
                return True
            elif self.readyQueue[0].cpuBurst == self.cpuProcess.cpuBurst:
                if self.readyQueue[0].arrivalTime < self.cpuProcess.arrivalTime:
                    return True
                elif self.readyQueue[0].arrivalTime == self.cpuProcess.arrivalTime:
                    if self.readyQueue[0].ID < self.cpuProcess.ID:
                        return True
...
def dataSort3(data):
    ...
    for i in range(0, n - 1, 1):
        for j in range(n - 1, i, -1):
            if data[j].cpuBurst < data[j - 1].cpuBurst:
                data[j], data[j - 1] = data[j - 1], data[j]
            elif data[j].cpuBurst == data[j - 1].cpuBurst:
                if data[j].arrivalTime < data[j - 1].arrivalTime:
                    data[j], data[j - 1] = data[j - 1], data[j]
                elif data[j].arrivalTime == data[j - 1].arrivalTime:
                    if data[j].ID < data[j - 1].ID:
...
def runAll(fileName):
    ...
    while (nowTime < totalCPUBurst):
        ...
        x = state.hasGetNext()
        state.schedual(x, state.readyQueue)
        state.run()
