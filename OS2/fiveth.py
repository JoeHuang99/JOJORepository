...
class State: # 負責整體的狀態
    ...
    def schedual(self, readyQueue):
        if self.cpuProcess.cpuBurst <= 0 and len(readyQueue) == 0:
            pass
        elif self.cpuProcess.cpuBurst <= 0 and len(readyQueue) != 0:
            self.getNextJob()
    ...
    def run(self):
        ...
        for i in range(len(self.readyQueue)):
            self.readyQueue[i].wait = self.readyQueue[i].wait + 1
...
def dataSort3(data):
    ...
    for i in range(0, n - 1, 1):
        for j in range(n - 1, i, -1):
            if computeRatio(data[j].cpuBurst, data[j].wait) > computeRatio(data[j - 1].cpuBurst, data[j - 1].wait):
                data[j], data[j - 1] = data[j - 1], data[j]
            elif computeRatio(data[j].cpuBurst, data[j].wait) == computeRatio(data[j - 1].cpuBurst, data[j - 1].wait):
                if data[j].arrivalTime < data[j - 1].arrivalTime:
                    data[j], data[j - 1] = data[j - 1], data[j]
                elif data[j].arrivalTime == data[j - 1].arrivalTime:
                    if data[j].ID < data[j - 1].ID:
...
def computeRatio(cpuBurstTime, wait):
    ratio = ( cpuBurstTime + wait ) / cpuBurstTime
    return ratio
...
def runAll(fileName):
    ...
    while (nowTime < totalCPUBurst):
        ...
        state.schedual(state.readyQueue)
        state.run()
