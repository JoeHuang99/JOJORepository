...
class State: # 負責整體的狀態
    ...
    def schedual(self, timeSlice, readyQueue):
        ...
        if self.cpuProcess.ID == -1 and len(readyQueue) != 0: # CPU的位置一開始是空的
            self.getNextJob()
            timeSliceTemp = int(timeSlice)
        elif timeSliceTemp == 0 and len(readyQueue) != 0 and self.cpuProcess.cpuBurst != 0: # 時間片段用完但還沒有做完且readyQueue還有人在等待
            readyQueue.append(self.cpuProcess)
            self.getNextJob()
            timeSliceTemp = int(timeSlice)
        elif timeSliceTemp == 0 and len(readyQueue) == 0 and self.cpuProcess.cpuBurst != 0: # 時間片段用完但還沒有做完且readyQueue沒有人在等待
            readyQueue.append(self.cpuProcess)
            self.getNextJob()
            timeSliceTemp = int(timeSlice)
        elif self.cpuProcess.cpuBurst == 0 and len(readyQueue) != 0: # 做完後且readyQueue還有人等待
            self.getNextJob()
            timeSliceTemp = int(timeSlice)
        elif self.cpuProcess.ID == -1 and len(readyQueue) == 0:
            pass
        else:
            pass
